...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        $secode = $this->redis->GET("ImgCode_" . $key);
        //  $secode = json_encode($result);
        if (empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if (NOW_TIME - $secode['verify_time'] > $this->expire) {
            $this->redis->delete("ImgCode_" . $key); // session($key, null);
            return false;
        }
        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && $this->redis->delete("ImgCode_" . $key);
            return true;
        }
        $this->redis->delete("ImgCode_" . $key);
        return false;
    }
    ...
    public function entry($id = '') {
        ...
        $this->redis->SET("ImgCode_" . $key . $id, json_encode($secode), 60);
