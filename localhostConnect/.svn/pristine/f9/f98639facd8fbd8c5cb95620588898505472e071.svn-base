...
use Common\Common\PhpAmqpLib\Exception\AMQPBasicCancelException;
use Common\Common\PhpAmqpLib\Exception\AMQPProtocolChannelException;
...
use Common\Common\PhpAmqpLib\Wire\AMQPWriter;
...
class AMQPChannel extends AbstractChannel {
    ...
    /**
     * @param \Common\Common\PhpAmqpLib\Connection\AbstractConnection $connection
    ...
    /**
     * @param AMQPReader $reader
     * @throws \Common\Common\PhpAmqpLib\Exception\AMQPProtocolChannelException
     */
    ...
    $realm, $exclusive = false, $passive = false, $active = false, $write = false, $read = false
    ) {
     ...
     * @return mixed|null
     */
    public function exchange_declare($exchange, $type, $passive = false, $durable = false, $auto_delete = true, $internal = false, $nowait = false, $arguments = null, $ticket = null) {
    ...
    $exchange, $if_unused = false, $nowait = false, $ticket = null
    ) {
        $ticket = $this->getTicket($ticket);
    ...
    $destination, $source, $routing_key = '', $nowait = false, $arguments = null, $ticket = null
    ) {
        $arguments = $this->getArguments($arguments);
        $ticket = $this->getTicket($ticket);

        list($class_id, $method_id, $args) = $this->protocolWriter->exchangeBind(
                $ticket, $destination, $source, $routing_key, $nowait, $arguments
        );
    ...
    public function exchange_unbind($destination, $source, $routing_key = '', $arguments = null, $ticket = null) {
        ...
        list($class_id, $method_id, $args) = $this->protocolWriter->exchangeUnbind(
                $ticket, $destination, $source, $routing_key, $arguments
        );
    ...
    protected function basic_ack_from_server(AMQPReader $reader) {
        ...
        if (!isset($this->published_messages[$delivery_tag])) {
            throw new AMQPRuntimeException(sprintf(
                    'Server ack\'ed unknown delivery_tag "%s"', $delivery_tag
            ));
        }
    ...
    protected function basic_nack_from_server($reader) {
        ...
        if (!isset($this->published_messages[$delivery_tag])) {
            throw new AMQPRuntimeException(sprintf(
                    'Server nack\'ed unknown delivery_tag "%s"', $delivery_tag
            ));
        }
    ...
    /**
     * @param AMQPReader $reader
     * @throws \Common\Common\PhpAmqpLib\Exception\AMQPBasicCancelException
     */
     ...
     * @param string $queue
     * @param string $consumer_tag
     * @param bool $no_local 设置为TRUE时服务器不接受此订阅者发布的消息。
     * @param bool $no_ack
     * @param bool $exclusive
     * @param bool $nowait   设置为TRUE时不需要等待服务器响应，可能会引发channel异常。
    ...
    public function basic_consume($queue = '', $consumer_tag = '', $no_local = false, $no_ack = false, $exclusive = false, $nowait = false, $callback = null, $ticket = null, $arguments = array()) {
        ...
        list($class_id, $method_id, $args) = $this->protocolWriter->basicConsume(
                $ticket, $queue, $consumer_tag, $no_local, $no_ack, $exclusive, $nowait, $this->protocolVersion == '0.9.1' ? $arguments : null
        );
        $this->send_method_frame(array($class_id, $method_id), $args);
        if (false === $nowait) {
    ...
    private function pre_publish($exchange, $routing_key, $mandatory, $immediate, $ticket) {
        $cache_key = sprintf(
                '%s|%s|%s|%s|%s', $exchange, $routing_key, $mandatory, $immediate, $ticket
        );
    ...
    public function basic_publish($msg, $exchange = '', $routing_key = '', $mandatory = false, $immediate = false, $ticket = null) {
        ...
        $this->connection->send_content(
                $this->channel_id, 60, 0, mb_strlen($msg->body, 'ASCII'), $msg->serialize_properties(), $msg->body, $pkt
        );
    ...
    $msg, $exchange = '', $routing_key = '', $mandatory = false, $immediate = false, $ticket = null
    ) {
    ...
    public function set_return_listener($callback) {
        if (!is_callable($callback)) {
            throw new \InvalidArgumentException(sprintf(
                    'Given callback "%s" should be callable. %s type was given.', $callback, gettype($callback)
            ));
        }
