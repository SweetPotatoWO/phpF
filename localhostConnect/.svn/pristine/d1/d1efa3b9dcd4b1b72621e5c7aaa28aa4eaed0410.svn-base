...
namespace Common\Common\Jpush;

use Common\Common\Jpush\Exceptions\APIConnectionException;
use Common\Common\Jpush\Exceptions\APIRequestException;
use Common\Common\Jpush\Config;
...
final class Http {
    ...
    private static function sendRequest($client, $url, $method, $body = null, $times = 1) {
        logger_wge("Send " . $method . " " . $url . ", body:" . $body . ",times:" . $times, "request", "jpush", "jpush");
        ...
        if ($errorCode) {
            ...
            if ($times < $retries) {
                logger_wge("curl_errno 返回的错误码为 ：" . $errorCode, "request", "crl", "jpush");
