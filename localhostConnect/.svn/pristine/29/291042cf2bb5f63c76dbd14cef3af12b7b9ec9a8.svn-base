...
+function($) {
    ...
    $.showLoading = function(text) {
        ...
        html += '</div>';
        html += '<p class="weui-toast_content pt_65 pl_5">' + (text || "数据加载中") + '</p>';
...
+function($) {
    ...
    PTR.prototype.touchEnd = function() {
        this.start = false;
        if (this.diffY <= 0 || this.container.hasClass("refreshing"))
            return;
...
+function($) {
    ...
    var Picker = function(params) {
        ...
        function isPopover() {
            var toPopover = false;
            if (!p.params.convertToPopover && !p.params.onlyInPopover)
                return toPopover;
            if (!p.inline && p.params.input) {
                if (p.params.onlyInPopover)
                    toPopover = true;
                else {
                    if ($.device.ios) {
        ...
        function inPopover() {
            if (p.opened && p.container && p.container.length > 0 && p.container.parents('.popover').length > 0)
                return true;
            else
                return false;
        }
        ...
        p.destroyPickerCol = function(colContainer) {
            ...
            if ('f7DestroyPickerCol' in colContainer[0])
                colContainer[0].f7DestroyPickerCol();
        };
        // Resize cols
        function resizeCols() {
            if (!p.opened)
        ...
        function closeOnHTMLClick(e) {
            ...
            if (p.input && p.input.length > 0) {
                ...
                if ($(e.target).parents('.weui-picker-modal').length === 0)
                    p.close();
            }
        }
...
+function($) {
    ...
    Select.prototype.parseInitValue = function() {
        ...
        if (!this._init && (value === undefined || value == null || value === ""))
            return;
    ...
    Select.prototype.open = function(values, titles) {
        ...
        for (var i = 0; i < selects.length; i++) {
            ...
            if (s._open) {
                if (!s.close())
                    return false; // 其他的select由于某些条件限制关闭失败。
            }
        }
    ...
    $.fn.select = function(params, args) {
        ...
        return this.each(function() {
            var $this = $(this);
            if (!$this.data("weui-select"))
                $this.data("weui-select", new Select(this, params));

            var select = $this.data("weui-select");

            if (typeof params === typeof "a")
                select[params].call(select, args);
    ...
    var Calendar = function(params) {
        ...
        p.updateValue = function() {
            ...
            if (p.input && p.input.length > 0) {
                if (p.params.formatValue)
                    inputValue = p.params.formatValue(p, p.value);
                else {
                    inputValue = [];
                    for (i = 0; i < p.value.length; i++) {
        ...
        p.initCalendarEvents = function() {
            ...
            function handleDayClick(e) {
                ...
                if (day.hasClass('picker-calendar-day-disabled'))
                    return;
                if (day.hasClass('picker-calendar-day-next'))
                    p.nextMonth();
                if (day.hasClass('picker-calendar-day-prev'))
                    p.prevMonth();
                ...
                if (p.params.closeOnSelect && !p.params.multiple)
                    p.close();
            }
        ...
        p.destroyCalendarEvents = function(colContainer) {
            if ('f7DestroyCalendarEvents' in p.container[0])
        ...
        p.monthHTML = function(date, offset) {
            ...
            for (i = 1; i <= rows; i++) {
                ...
                for (j = 1; j <= cols; j++) {
                    ...
                    // Today
                    if (dayDate === today)
                        addClass += ' picker-calendar-day-today';
                    // Selected
        ...
        p.setMonthsTranslate = function(translate) {
            ...
            if (typeof p.monthsTranslate === 'undefined')
                p.monthsTranslate = translate;
            p.months.removeClass('picker-calendar-month-current picker-calendar-month-prev picker-calendar-month-next');
        ...
        p.setYearMonth = function(year, month, transition) {
            ...
            if (targetDate > currentDate) {
                ...
                p.monthsTranslate--;
                if (!p.animating)
                    p.months.eq(p.months.length - 1).remove();
        ...
        p.layout = function() {
            ...
            if (!p.inline)
                pickerClass = 'weui-picker-modal ' + pickerClass;
        ...
        if (!p.inline)
            $(document).on('click touchend', closeOnHTMLClick);
...
+function($) {
    ...
    Datetime.prototype = {
        ...
        getConfig: function() {
            ...
            if (inputValue)
                config.value = params.parse(inputValue);
    ...
    $.fn.datetimePicker = function(params) {
        ...
        return this.each(function() {
            ...
            if (!datetime)
                $this.data("datetime", new Datetime(this, params));
            return datetime;
        });
    };
    ...
    $(document).on("click", ".close-popup, .weui-popup__overlay", function() {
        $.closePopup();
            ...
            .on("click", ".weui-popup__container", function(e) {
                if ($(e.target).hasClass("weui-popup__container"))
                    $.closePopup();
            })
    ...
    $.fn.slider = function(arg) {
        this.each(function() {
            ...
            if (slider)
                ...
                $this.data('slider', new Slider(this, arg))
        })
    }
}($);
