...
class GraphDbHelper
{
    ...
    public function findOneBy($class,array $criteria){
        return $this->db->getRepository($class)->findOneBy($criteria);
    }
    ...
    public function query($q,$limit='',$mapping=null,$param=null)
    {
        ...
        try {
            ...
            if(APP_DEBUG){
                logger('GraphDbHelper/query执行完成',json_encode([
                    '语句'=>$q,
                    '参数'=>$param
            ...
            logger('GraphDbHelper/query异常',json_encode([
                '错误'=>$ex->getMessage(),
                '语句'=>$q
            ],JSON_UNESCAPED_UNICODE));
            return null;
    ...
    public function execute($q,$param=null)
    {
        ...
        try {
            $query->execute();
            if(APP_DEBUG){
                logger('GraphDbHelper/execute执行完成',json_encode([
                    '语句'=>$q,
                    '参数'=>$param
                ]));
            }
            ...
            logger('GraphDbHelper/execute异常',json_encode([
                '错误'=>$ex->getMessage(),
                '语句'=>$q
            ]));
            return false;
