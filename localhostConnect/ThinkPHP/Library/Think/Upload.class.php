...
class Upload {
    ...
    private function check($file) {
        ...
        if (getimagesize($file['tmp_name']) && !$this->checkHex($file['tmp_name'])) {
            $this->error = '未能通过安全检查的文件！';
            return false;
        }
        /* 检查文件后缀 */
    ...
    private function getSubPath($filename) {
        ...
        if ($this->autoSub && !empty($rule)) {
            if (strlen($this->subNamePath) > 1) {
                $subpath = $this->getName($rule, $filename) . '/' . $this->subNamePath;
            } else {
    ...
    public function checkHex($file) {
        if (file_exists($file)) {
            ...
            if (preg_match("/(3c25.*?28.*?29.*?253e)|(3c3f.*?28.*?29.*?3f3e)|(3c3f706870)|(3C534352495054)|(2F5343524950543E)|(3C736372697074)|(2F7363726970743E)/is", $hexCode)) {
                $this->error(5);
                return false;
            } else {
                return true;
